...
var LocalPackage = (function (_super) {
    ...
    LocalPackage.prototype.install = function (installSuccess, errorCallback, installOptions) {
        ...
        try {
            ...
            GetFilePathUtil.getDataDirectory(LocalPackage.DownloadUnzipDir, false, function (error, directoryEntry) {
                var unzipPackage = function () {
                    GetFilePathUtil.getDataDirectory(LocalPackage.DownloadUnzipDir, true, function (innerError, unzipDir) {
                        if (innerError) {
                ...
                if (!error && !!directoryEntry) {
                    directoryEntry.removeRecursively(function () {
                        ...
                        installError && installError(GetFilePathUtil.fileErrorToError(cleanupError));
                    });
                }
                else {
                    unzipPackage();
    ...
    LocalPackage.handleDeployment = function (newPackageLocation, deployCallback) {
        GetFilePathUtil.getDataDirectory(newPackageLocation, true, function (deployDirError, deployDir) {
            GetFilePathUtil.getDataFile(LocalPackage.DownloadUnzipDir, LocalPackage.DiffManifestFile, false, function (manifestError, diffManifest) {
    ...
    LocalPackage.handleCleanDeployment = function (newPackageLocation, cleanDeployCallback) {
        GetFilePathUtil.getDataDirectory(newPackageLocation, true, function (deployDirError, deployDir) {
            GetFilePathUtil.getDataDirectory(LocalPackage.DownloadUnzipDir, false, function (unzipDirErr, unzipDir) {
                if (unzipDirErr || deployDirError) {
                    cleanDeployCallback(new Error("Could not copy new package."), null);
                }
                else {
                    GetFilePathUtil.copyDirectoryEntriesTo(unzipDir, deployDir, function (copyError) {
                        if (copyError) {
        ...
        var doCopy = function (currentPackagePath) {
            ...
            if (currentPackagePath) {
                getCurrentPackageDirectory = function (getCurrentPackageDirectoryCallback) {
                    GetFilePathUtil.getDataDirectory(currentPackagePath, false, getCurrentPackageDirectoryCallback);
                };
            }
            else {
                newPackageLocation = newPackageLocation + "/www";
                getCurrentPackageDirectory = function (getCurrentPackageDirectoryCallback) {
                    GetFilePathUtil.getApplicationDirectory("www", getCurrentPackageDirectoryCallback);
                };
            }
            GetFilePathUtil.getDataDirectory(newPackageLocation, true, function (deployDirError, deployDir) {
                if (deployDirError) {
                    ...
                    var success = function (currentPackageDirectory) {
                        GetFilePathUtil.copyDirectoryEntriesTo(currentPackageDirectory, deployDir, copyCallback);
                    };
                    var fail = function (fileSystemError) {
                        copyCallback && copyCallback(GetFilePathUtil.fileErrorToError(fileSystemError), null);
                    };
    ...
    LocalPackage.handleDiffDeployment = function (newPackageLocation, diffManifest, diffCallback) {
        ...
        LocalPackage.copyCurrentPackage(newPackageLocation, function (currentPackageError) {
            LocalPackage.handleCleanDeployment(newPackageLocation, function (cleanDeployError) {
                GetFilePathUtil.readFileEntry(diffManifest, function (error, content) {
                    if (error || currentPackageError || cleanDeployError) {
                        handleError(new Error("Cannot perform diff-update."));
                    }
                    else {
                        var manifest = JSON.parse(content);
                        GetFilePathUtil.deleteEntriesFromDataDirectory(newPackageLocation, manifest.deletedFiles, function (deleteError) {
                            GetFilePathUtil.getDataDirectory(newPackageLocation, true, function (deployDirError, deployDir) {
    ...
    LocalPackage.writeCurrentPackageInformation = function (packageInfoMetadata, callback) {
        ...
        GetFilePathUtil.writeStringToDataFile(content, LocalPackage.RootDir, LocalPackage.PackageInfoFile, true, callback);
    };
    LocalPackage.backupPackageInformationFile = function (callback) {
        var reportFileError = function (error) {
            callback(GetFilePathUtil.fileErrorToError(error), null);
        };
        ...
        var gotFile = function (error, currentPackageFile) {
            if (error) {
                ...
                GetFilePathUtil.getDataFile(LocalPackage.RootDir, LocalPackage.OldPackageInfoFile, false, function (error, oldPackageFile) {
        ...
        GetFilePathUtil.getDataFile(LocalPackage.RootDir, LocalPackage.PackageInfoFile, false, gotFile);
    };
    ...
    LocalPackage.getPackage = function (packageFile, packageSuccess, packageError) {
        ...
        try {
            GetFilePathUtil.readDataFile(LocalPackage.RootDir, packageFile, function (error, content) {
                if (error) {
                    handleError(error);
                }
                else {
