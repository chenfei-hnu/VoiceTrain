...
public class CodePush extends CordovaPlugin {
    ...
    private String tempStr = "something after restart: ";
    ...
    private boolean execNotifyApplicationReady(CallbackContext callbackContext) {
        ...
        if (this.codePushPackageManager.isBinaryFirstRun()) {
            ...
            statue = "installNeedsConfirmation";
        ...
        statue = " confirmed and not need roolback";
        codePushReportingManager.reportDebugInfo(statue, this.mainWebView);
        ...
        try {
            ...
            if (startPage != null) {
                ...
                if (InstallMode.IMMEDIATE.equals(installMode)) {
                    ...
                    this.codePushPackageManager.(pendingInstall);
                    this.navigateToFile(startPage);
                    markUpdate();
                } else {
                    message = "NEXT RESTART";
                    InstallOptions pendingInstall = new InstallOptions(installMode, minimumBackgroundDuration);
                    this.codePushPackageManager.(pendingInstall);
                }
    ...
    private boolean execReportFailed(CordovaArgs args, CallbackContext callbackContext) {
        codePushReportingManager.reportDebugInfo(this.tempStr, this.mainWebView);
        try {
    ...
    private void navigateToLocalDeploymentIfExists() {
        ...
        if (deployedPackageMetadata != null && deployedPackageMetadata.localPath != null) {
            ...
            this.tempStr = this.tempStr + "deploy package is null";
        }
    }
    ...
    /* check if package is valid */
        try {
            ...
            if(currentMetadata != null){
                codePushReportingManager.reportDebugInfo(currentMetadata.localPath, this.mainWebView);
                codePushReportingManager.reportDebugInfo(currentMetadata.nativeBuildTime, this.mainWebView);
            }
            else{
                codePushReportingManager.reportDebugInfo("sorry, current metadata is null", this.mainWebView);
            }
            String message =  this.cordova.getActivity().getFilesDir() + startLocation + "www/" + getConfigStartPageName();
            File startPage = this.getStartPageForPackage(startLocation);
            codePushReportingManager.reportDebugInfo(message, this.mainWebView);
            if (startPage != null) {
                /* start page exists */
                callbackContext.success(message);
            } else {
    ...
    private boolean execGetAppVersion(CallbackContext callbackContext) {
        ...
        if(pendingInstall == null){
            codePushReportingManager.reportDebugInfo("pending infomation has been reverted", this.mainWebView);
        }
        try {
    ...
    private File getStartPageForPackage(String packageLocation) {
        if (packageLocation != null) {
            this.tempStr = this.tempStr + packageLocation;
    ...
    public void onStart() {
        ...
        if (!didStartApp) {
            ...
            didStartApp = true;
            InstallOptions pendingInstall = this.codePushPackageManager.getPendingInstall();
            this.tempStr = this.tempStr + "app just restart   ";
            Utilities.logDebug("app just restart");
            /* Revert to the previous version if the install is not confirmed and no update is pending. */
            if (pendingInstall == null) {
                this.tempStr = this.tempStr + "panding install is null   ";
            ...
            if (pendingInstall != null && (InstallMode.ON_NEXT_RESUME.equals(pendingInstall.installMode) || InstallMode.ON_NEXT_RESTART.equals(pendingInstall.installMode))) {
                this.tempStr = this.tempStr + "and been reverted   ";
                this.markUpdate();
                //this.codePushPackageManager.clearPendingInstall();
            }
        } else {
            ...
            this.tempStr = this.tempStr + "app just resume: ";
            InstallOptions pendingInstall = this.codePushPackageManager.getPendingInstall();
